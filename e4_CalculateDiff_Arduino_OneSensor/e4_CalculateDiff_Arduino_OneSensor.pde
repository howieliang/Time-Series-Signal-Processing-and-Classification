...
void draw() {
  ...
  for (int i = 0; i < sensorNum; i++) {
    lineGraph(sensorHist[i], 0, height, 0, 0, width, height*0.5, i); //history of signal
    lineGraph(diffArray[i], -height, height, 0, height*0.5, width, height*0.5, diffMode); //history of signal
  }
}
...
void serialEvent(Serial port) {   
  ...
  if (!b_pause) {
    ...
    if (dataIndex>=0) {
      ...
      float diff = sensorHist[dataIndex][0] - sensorHist[dataIndex][1]; //normal diff
      if(diffMode==1) diff = abs(sensorHist[dataIndex][0] - sensorHist[dataIndex][1]); //absolute diff
      appendArray(diffArray[dataIndex], diff);
      return;
    }
  }
}
