...
// e5_Activiation_Arduino_OneSensor
...
void draw() {
  ...
  for (int i = 0; i < sensorNum; i++) {
    ...
    lineGraph(diffArray[i], -height, height, 0, height*0.5, width, height*0.5, i); //history of signal
  }
...
void serialEvent(Serial port) {   
  ...
  if (!b_pause) {
    ...
    if (dataIndex>=0) {
      ...
      appendArray(diffArray[dataIndex], diff); //store the data to history (for visualization)
