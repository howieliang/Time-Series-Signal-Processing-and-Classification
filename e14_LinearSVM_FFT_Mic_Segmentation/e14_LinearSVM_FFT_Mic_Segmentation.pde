...
void draw()
{
  ...
  updateFFT(samples);
  energySum = 0; //reset the measurement of energySum
  for (int i = 0; i < HIGH_THLD-LOW_THLD; i++) {
    energySum+= fft.getBand(i+LOW_THLD);
  }
  ...
  drawSpectrogram();
  appendArrayTail(energyHist, energySum); //the class is null without mouse pressed.
...
void drawSpectrogram() {
  ...
  for (int i = 0; i < HIGH_THLD-LOW_THLD; i++) {
    ...
    energySum+=FFTHist[i][col]; //calculate the sum of power of each spectrum
  }
